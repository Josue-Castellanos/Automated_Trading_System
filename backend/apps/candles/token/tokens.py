...
class Tokens:
    ...
    def _check_schwab_keys(self):
        ...
        """
        if (
            len(self.settings.APP_KEY) != 32
            or len(self.settings.SECRET_KEY) != 16
        ):
            quit()
    ...
    def _check_schwab_tokens(self):
        ...
        """
        if (datetime.now() - self.refreshTokenDateTime).days >= (
            self.refreshTokenTimeout - 1
        ):
            self._update_refresh_token()
        elif ((datetime.now() - self.accessTokenDateTime).days >= 1) or (
            (datetime.now() - self.accessTokenDateTime).seconds
            > (self.accessTokenTimeout - 60)
        ):
            self._access_token()
    ...
    def _post_access_token(self, grant_type, code):
        ...
        return requests.post(
            self.settings.POST_ENDPOINT, headers=headers, data=data
        ).json()
    ...
    def _access_token(self):
        ...
        (
            ...
            tokenDictionary,
        ) = self._schwab_token_manager("getFile")
        self._schwab_token_manager(
            "set",
            datetime.now(),
            refreshTokenFileTime,
            self._post_access_token(
    ...
    def _update_refresh_token(self):
        """
        Obtain a new refresh token by opening
        the authorization URL in a web browser.
        """
        authUrl = (
            f"https://api.schwabapi.com/v1/oauth/authorize"
            f"?response_type=code&client_id={self.settings.APP_KEY}"
            f"&redirect_uri={self.settings.CALLBACK_URL}"
        )
        ...
        self._schwab_token_manager(
            "set", datetime.now(), datetime.now(), tokenDictionary
        )

    def _schwab_token_manager(
        self,
        todo=None,
        ...
        """
        Manage token operations such as reading,
        writing, and updating token information using dotenv.
        ...
        Returns:
            tuple or None:
                Depending on the operation, may return token information.
        """
        ...
        def write_token_file(
            new_access_token_time, new_refresh_token_time, new_token_dict
        ):
            """Update .env file with new token information using dotenv."""
            # Convert datetime objects to strings
            access_time_str = new_access_token_time.strftime(
                access_token_time_format
            )
            refresh_time_str = new_refresh_token_time.strftime(
            ...
            set_key(
                self.settings.ENV_PATH,
                "REFRESH_TOKEN_DATETIME",
                refresh_time_str
            )
            set_key(
                self.settings.ENV_PATH,
                "ACCESS_TOKEN",
                new_token_dict.get("access_token"),
            )
            set_key(
                self.settings.ENV_PATH,
                "REFRESH_TOKEN",
                new_token_dict.get("refresh_token"),
            )
            set_key(
                self.settings.ENV_PATH,
                "ID_TOKEN",
                new_token_dict.get("id_token")
            )
            set_key(
                self.settings.ENV_PATH,
                "JSON_DICT",
                json.dumps(new_token_dict)
            )
