...
class Schwab:
    def __init__(self):
        ...
        This constructor sets up the necessary attributes
        and performs initial checks.
        """
        self.timeout = 5
    ...
    def account_numbers(self):
        ...
        Returns:
            requests.Response: The API response containing
            account numbers and hashes.

        Note:
            Account numbers in plain text cannot be used outside
            of headers or request/response bodies.
            This method should be invoked first to retrieve
    ...
    def accounts(self, fields=None):
        ...
        Returns:
            requests.Response: response containing
            details for all linked accounts.
    ...
    def account_number(self, accountNumber=None, fields=None):
        ...
        Args:
            accountNumber (str): Account hash.
            fields (str, optional): Fields to return.

        Returns:
            requests.Response: containing details for the specified account.

        Note:
            Balance information is displayed by default.
    ...
    def account_orders(
        ...
        Returns:
            requests.Response: containing orders for the specified account.
                ...
                params=self._params_parser(
                    {
                        ...
                        "fromEnteredTime": self._time_converter(
                            fromEnteredTime, format="iso"
                        ),
                        "toEnteredTime": self._time_converter(
                            toEnteredTime, format="iso"
                        ),
                        "status": status,
                    }
                ),
    ...
    def post_orders(self, order, accountNumber=None):
        ...
        Args:
            ...
            accountNumber (str, optional): Account hash.

        Returns:
            requests.Response: containing the order number.
    ...
    def get_chains(
        ...
        Args:
            ...
            contractType (str, optional): Type of contracts to return
                ("CALL", "PUT", "ALL").
            strikeCount (int, optional): The number of strikes to return,
                 above and below the current price.
            includeQuotes (bool, optional): Include quotes for options.
            strategy (str, optional): Option strategy type.
            interval (float, optional): Strike interval for spread strategy.
            ...
            entitlement (str, optional): Entitlement.
    ...
    def order_replace(self, accountNumber, orderId, order):
        ...
        Returns:
            requests.Response: results of the order replacement.
    ...
    def all_orders(self,
        ...
        Returns:
            requests.Response: containing all orders across all accounts.
        Note:
        ...
        try:
            ...
            return {
                "error": f"Failed to retrieve all accounts orders: {str(e)}"
            }
    ...
    def transactions(
        ...
        Returns:
            requests.Response: containing the list of transactions.
            ...
            params=self._params_parser(
                {
                    ...
                    "startDate": self._time_converter(startDate, format="iso"),
                    "endDate": self._time_converter(endDate, format="iso"),
                    "symbol": symbol,
                    "types": types,
                }
            ),
    ...
    def preferences(self):
        ...
        try:
            response = requests.get(
                f"{self.settings.ACCOUNT_ENDPOINT}/userPreference",
                headers=self._get_headers(),
                timeout=self.timeout,
            )
            response.raise_for_status()
            ...
            return {
                "error": f"Failed to retrieve stream information: {str(e)}"
            }
                ...
                params=self._params_parser(
                    {
                        ...
                        "frequency": frequency,
                        "startDate": self._time_converter(
                            startDate,
                            format="epoch"
                        ),
                        "endDate": self._time_converter(
                            endDate,
                            format="epoch"
                        ),
    ...
    def market_hours(self, symbols, date=None):
        ...
        Args:
            symbols (list):
                List of market symbols (e.g., "equity", "option", "future").
            date (datetime | str, optional):
                The date to retrieve market hours for.
    ...
    def market_hour(self, market_id, date=None):
        ...
        Args:
            market_id (str):
                The market identifier (e.g., "equity", "option", "future").
            date (datetime | str, optional):
                The date to retrieve market hours for.
    ...
    def instruments(self, symbol, projection):
        ...
        This method allows searching for financial
        instruments using various projection types.

        Args:
            symbol (str): The symbol or search string to use
            for the instrument search.
            projection (str): The type of search to perform.
                Valid options are:
                - "symbol-search":
                    Retrieve instrument data for a specific symbol or cusip.
                - "symbol-regex":
                    Retrieve instrument data for all symbols matching regex.
                - "desc-search":
                    Retrieve instrument data for instruments
                    whose description contains the word supplied.
                - "desc-regex":
                    Retrieve instrument data for instruments
                    whose description matches the supplied regex.
                - "fundamental":
                    Retrieve fundamental data for a specific symbol.

        Returns:
            requests.Response: containing the instrument
        ...
        if projection not in valid_projections:
            raise ValueError(
                f"Invalid projection type. Must be one of {valid_projections}"
            )

        return requests.get(
            f"{self.settings.MARKET_ENDPOINT}/instruments",
            headers={
                "Authorization": f"Bearer {self.settings.ACCESS_TOKEN}"
            },
    ...
    def instrument_cusip(self, cusip_id):
        """
        Retrieve instrument data for a specific CUSIP
        (Committee on Uniform Securities Identification Procedures) identifier.

        This method fetches detailed information about a financial instrument
          using its CUSIP number.

        Args:
            cusip_id (str): The CUSIP identifier of the instrument.

        Returns:
            requests.Response: containing detailed instrument data
                for the specified CUSIP.

        Note:
            CUSIP is a nine-character alphanumeric code that
