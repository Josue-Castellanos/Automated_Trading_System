...
def update_subscription_status(sender, instance, created, **kwargs):
    ...
    if created and instance.status == 'successful' and instance.subscription:
        ...
        if subscription.status == 'inactive':
            ...
            subscription.save()
            logger.info(f"Subscription reactivated for user {instance.user.email}")
...
def ensure_default_payment_method(sender, instance, created, **kwargs):
    ...
    """
    if created and not PaymentMethod.objects.filter(user=instance.user, is_default=True).exists():
        instance.is_default = True
        instance.save()
        logger.info(f"Default payment method set for user {instance.user.email}")
...
def update_profile_payment_status(sender, instance, created, **kwargs):
    ...
    if created and instance.status == 'successful':
        ...
        profile.save()
        logger.info(f"Profile payment status updated for user {instance.user.email}")