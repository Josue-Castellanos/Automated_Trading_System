...
def stochastic_hull(data, k_period=8, d_period=2, overbought=90, oversold=10):
    ...
    data["stoch_strength"] = (
        data["stoch_d"].rolling(3).apply(calculate_strength, raw=True)
    )

    # Detect breakouts
    data["stoch_overbought"] = data["stoch_d"] >= overbought
    data["stoch_oversold"] = data["stoch_d"] <= oversold
    data["stoch_bullish_break"] = (data["stoch_d"] > overbought) & (
        data["stoch_d"].shift(1) <= overbought
    )
    data["stoch_bearish_break"] = (data["stoch_d"] < oversold) & (
        data["stoch_d"].shift(1) >= oversold
    )
    data["stoch_bearish_reverse"] = (data["stoch_d"] > oversold) & (
        data["stoch_d"].shift(1) <= oversold
    )
    data["stoch_bullish_reverse"] = (data["stoch_d"] < overbought) & (
        data["stoch_d"].shift(1) >= overbought
    )
    ...
    data["stoch_color"] = np.where(
        data["stoch_signal"].str.contains("bull"),
        "lime",
        np.where(data["stoch_signal"].str.contains("bear"), "red", "gray"),
    )
    return data
