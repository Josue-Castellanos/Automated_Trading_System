...
class TrendAdvisorService:
    ...
    def analyze_trend(data):
        ...
        for ma, ma_values in [('50', ma_50_values), ('200', ma_200_values)]:
            df[f'MA_{ma}_slope'] = ma_values.rolling(window=20).apply(
                lambda x: np.polyfit(range(20), x, 1)[0] if not all(pd.isna(x)) else float('nan'), 
                raw=True
            )
        ...
        current = {
            ...
            'ma_50': float(ma_50_values.iloc[-1]) if not pd.isna(ma_50_values.iloc[-1]) else None,
            'ma_200': float(ma_200_values.iloc[-1]) if not pd.isna(ma_200_values.iloc[-1]) else None,
            'ma_50_slope': float(df['MA_50_slope'].iloc[-1]) if not pd.isna(df['MA_50_slope'].iloc[-1]) else None,
            'ma_200_slope': float(df['MA_200_slope'].iloc[-1]) if not pd.isna(df['MA_200_slope'].iloc[-1]) else None
        }
    ...
    def _determine_phase(current):
        ...
        for phase, info in TrendAdvisorService.PHASES.items():
            ...
            slope_match = True
            for ma, expected in [('50', conditions['50_slope']), ('200', conditions['200_slope'])]:
                ...
                if expected == 'positive' and not current_slope.startswith('positive'):
                    slope_match = False
                elif expected == 'negative' and not current_slope.startswith('negative'):
                    slope_match = False
                elif expected == 'neutral' and (current_slope.startswith('positive') or current_slope.startswith('negative')):
                    slope_match = False
            
            # Check price relation
            price_match = False
            if conditions['price_relation'] == 'above_both' and price_relation == 'above_both':
                price_match = True
            elif conditions['price_relation'] == 'above_50' and price_relation in ['above_50', 'above_both']:
                price_match = True
            elif conditions['price_relation'] == 'below_50' and price_relation in ['below_50_above_200', 'below_both']:
                price_match = True
            elif conditions['price_relation'] == 'below_both' and price_relation == 'below_both':
                price_match = True
            elif conditions['price_relation'] == 'crossing_50' and price_relation in ['above_50', 'below_50_above_200']:
                price_match = True
            
            if slope_match and price_match:
                current_step = 0
                if (phase == 'Bullish' and 
                    price_relation == 'above_both' and 
                    current_50_slope == 'positive' and 
                    current_200_slope == 'positive'):
                    current_step = 1
    ...
    def _get_options_strategy(phase, step):
        ...
        return {
            'strategy': strategies.get(phase, {}).get(step, "No options action recommended"),
            'greeks': {
